 ...
 * `ErrorBanner` renders through this component, and the record names this application
 * interpolates into its own translated sentences - cannot become active DOM.
 *
 ...
 * reconciliation error dialog, the five list/report intro sentences that interpolate a record
 * name, and the link-search result descriptions - is protected by one shared allow-list instead
 * of ~20 separate decisions.
 */
...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
    return <ReactMarkdown
        ...
        // `rehypeSanitizeMarkup` runs AFTER `rehypeRaw` on purpose: raw HTML has to be parsed into
        // real nodes before it can be filtered, and filtering the parsed tree is what makes the
        // allow-list authoritative over whatever the untrusted string happened to spell.
